...
# Instant fixer removed during rollback
instant_fixer = None
...
from correttore.services.parallel_processor import get_parallel_corrector, ProcessingTask
...
# ——— Local modules ————————————————————————————————
from correttore.utils.reports import write_glossary_report, write_markdown_report
...
from correttore.utils.token_utils import NAME_RE, WORD_RE
...
async def correggi_footnotes_xml_async(docx_path: Path,
    ...
    if not os.path.exists(footnotes_file):
        logger.info("📝 Nessun file footnotes.xml trovato - documento senza note")
        shutil.rmtree(tmp_dir)
        return                              # documento senza note
    
    # Se il file esiste ma è vuoto, non ci sono note → esci
    if os.path.getsize(footnotes_file) == 0:
        logger.info("📝 File footnotes.xml vuoto - nessuna nota da correggere")
        shutil.rmtree(tmp_dir)
        return
    
    logger.info(f"📝 Trovato footnotes.xml ({os.path.getsize(footnotes_file)} bytes)")
    ...
    logger.info(f"📝 Trovate {len(footnotes)} note a piè di pagina da elaborare")
...
async def correct_paragraph_group(
    ...
    if safe_corrector is None:
        # Fallback con threshold abbassata a 0.55 per permettere più correzioni
        safe_corrector = SafeCorrector(conservative_mode=True, quality_threshold=0.55)
    ...
    for idx, p in enumerate(paragraphs):
        ...
        # 🚀 CORREZIONI IMMEDIATE PREMIUM - STEP 0: Disabled during rollback
        # instant_fixer was removed, so skip immediate corrections
        fixed_text = original
        instant_corrections = []
        
        # Skip to cache lookup since instant corrections are disabled
        
        # ═══ CACHE CHECK PRELIMINARE - FASE 4 ═══
        cached_entry = cache.get_with_similarity(original, threshold=0.98)
        if cached_entry and cached_entry.quality_score >= 0.55:  # Soglia abbassata per accettare più correzioni cached
        ...
        def spellcheck_func(text):
            # spellfix_paragraph was removed during rollback, return original text
            return text
        ...
        # ═══ STEP 3: AI CORRECTION (sempre, per catturare tutti gli errori) ═══
        # MODIFICA: Invia SEMPRE all'AI per non perdere correzioni, anche dopo spell/grammar
        # L'AI può trovare errori che spell/grammar non hanno trovato
        pending_idx.append(idx)
        pending_txt.append(current_text)
        # Note: La validazione successiva previene duplicazioni o peggioramenti
    ...
    try:
        ...
        for local_i, ai_corrected in zip(pending_idx, corrected_list):
            ...
            # 🚀 CORREZIONI IMMEDIATE PREMIUM - disabled during rollback
            fixed_text = original_para_text
            instant_corrections = []
