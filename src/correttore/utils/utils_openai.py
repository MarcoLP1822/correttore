...
from correttore.config.settings import OPENAI_MODEL, RETRY_BACKOFF, MAX_RETRY
...
async def _retry_async(call_coroutine, *, max_attempts, backoff_seq) -> List[Dict]:
    ...
    # Questo punto non dovrebbe mai essere raggiunto, ma aggiungiamo per sicurezza typing
    raise RuntimeError("Unexpected end of retry loop")

async def _chat_async(messages, client) -> List[Dict]:
    async def _one_call():
...
def get_corrections_sync(payload_json: str,
                         ...
                         context: str = "") -> List[Dict]:
    msgs = build_messages(context, payload_json, glossary)
    return asyncio.run(_chat_async(msgs, client))
